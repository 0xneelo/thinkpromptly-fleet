...
// hash of the sign-in password. Run it once, in a plain Terminal window of your own (not tmux,
// screen, cmux or zellij); no sudo. To rotate, delete the file and run it again, then restart the deck.
//
...
const { hashPassword, loadOperator, operatorFingerprint, OPERATOR_ID } = require('../operator-auth');
...
async function main() {
  ...
  // cmux's CLI can read any tab's scrollback; tmux, screen and zellij keep their own.
  if (['TMUX', 'STY', 'CMUX_WORKSPACE_ID', 'CMUX_SURFACE_ID', 'ZELLIJ'].some((k) => k in process.env))
    throw new Refusal(
      'run it in a plain Terminal window, not tmux, screen, cmux or zellij — the password is printed once and a multiplexer keeps scrollback any same-user process can read'
    );
  ...
  const written = loadOperator(text); // never write a file the deck would refuse to start on
  ...
  // Not secret: deck.log shows the same line at every boot, and a different one means a replaced file.
  console.log('operator fingerprint: ' + operatorFingerprint(written.identity));
}
