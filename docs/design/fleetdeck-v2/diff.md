...
| D08 | Reply toast | bottom-right 340px, "{from} replied", preview, Open thread, 7s auto-dismiss (L769-780, L1473) | none | S | P1 → P2 (L6) | S1, L6 | P1 ✅ S1 `70c32bb` (gate 36/36, max 0.033 %) · P2 ✅ L6.1 `2f72f00` (toast proven both ways; fixture gate 36/36, live 50/50) · L6.2 ✅ `eb3c015` (attach independent of load order, deep links live; weave `e646554`) |
...
| D18 | Landing | fixed nav (Deck¹⁴ · App · Registry · Certs · Docs · theme · Request access), hero (kickers, blurb, eyebrow, "Attach. Steer. Audit.", session widget "german-box · HOLDER OK · 12 ACTIVE · Open the deck"), 80vh spacer, Capability ("Registry On Demand", "Nothing runs unwatched.", 3 numbered cards), video-scrub canvas, reveal animations (L37-139, L969-1200) | none | L | P1 (free in S1) → P2 route (L1) | S1, L1 | P1 ✅ S1 `70c32bb` (gate 36/36, max 0.033 %) · P2 ✅ L11 `8f95eb2` (`/` → `?view=land` live) · rider L12 open (scroll-scrub video dead: server ignores Range; Hadwig on `agent-v2-l12`, decisions.md 2026-09-07 late) |
...
| D20 | View switching + back pill | root `state.view` land/app/deck, "← fleetdeck" back pill bottom-right (L965, L2124-2133) | separate pages | S | P2 (L1) | L1 | ✅ L11 `8f95eb2` + L11.1 `6e7f27f` (routes live, gate 36/36, live smoke 30/30) · L11.2 ✅ `2b7254b` (hash written on nav; weave `e646554`) |
