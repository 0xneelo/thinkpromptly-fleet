...
async function scenario(browser, port, opts, body) {
  const { theme = 'dark', sshkeys, ghtrain, offline = false, posts = [], gets = [] } = opts;
...
async function main() {
  ...
  try {
    ...
    allNoise.push(...await scenario(browser, server.port, { sshkeys: baseKeys, ghtrain: baseTrain }, async ({ page, keys }) => {
      ...
      await page.screenshot({ path: join(HERE, 'live-dark.png'), animations: 'disabled', caret: 'hide' });
    }));
    ...
    allNoise.push(...await scenario(browser, server.port, { sshkeys: baseKeys, ghtrain: baseTrain, theme: 'light' }, async ({ page, keys }) => {
      ...
      await page.screenshot({ path: join(HERE, 'live-light.png'), animations: 'disabled', caret: 'hide' });
    }));
    ...
    allNoise.push(...await scenario(browser, server.port, { sshkeys: baseKeys, ghtrain: { __status: 503, body: { ok: false, error: TRAIN_DOWN } } }, async ({ keys }) => {
      const train = card(keys, TRAIN);
    ...
    allNoise.push(...await scenario(browser, server.port, { sshkeys: baseKeys, ghtrain: { __status: 503, body: { ok: false } } }, async ({ keys }) => {
    ...
    // -- S6 offline --------------------------------------------------------
    allNoise.push(...await scenario(browser, server.port, { sshkeys: null, ghtrain: baseTrain, offline: true }, async ({ keys }) => {
    ...
    allNoise.push(...await scenario(browser, server.port, {
      ...
      eq('L7-55', 'load a train under an hour out', 'under an hour formats as mm:ss', await text(card(keys, TRAIN).locator('> div > span').nth(1)), '05:10');
    }));
    ...
    check('L7-60', 'watch the console across every scenario', 'the screen logs no errors', noise.length === 0, noise.join(' | '));
  } finally {
    await browser.close();
    await server.close();
  }
