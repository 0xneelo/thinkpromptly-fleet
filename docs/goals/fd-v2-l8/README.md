# fd-v2-l8 — Accounts (credits) cards

| | |
|---|---|
| Project / sub-project | `remote-system` / `fleetdeck-v2` |
| Worker | **Kunhild** · `frontend-developer` · tag `agent-kunhild` · Claude, high · `/goal` |
| Branch | `agent-v2-l8` off `origin/agent-v2-base`; first action `git merge --no-edit origin/agent-v2-l1` |
| Ledger rows | D15 (`docs/design/fleetdeck-v2/diff.md`) |
| Owned file | `public/v2/screens/accounts.js` + this screen's methods in `logic.js` |
| Data identifiers | accounts ← FD.data.toAccounts(credits) |
| API | FD.data.credits({refresh}) |
| Design seat | 🎨 DESIGN 35 — `docs/design/fleetdeck-v2/plan.md`, `decisions.md` |
| Registry group | `fd-v2` |

## Goal (one line)

Make the **Accounts (credits) cards** screen work on live data exactly like today's app (`BEHAVIOUR.md`), inside the mock's markup (pixel gate green in fixture mode), with every gap the mock leaves improvised in its style and documented.

## Inputs (read in this order)

1. `BEHAVIOUR.md` in this directory — the behaviour spec, item by item.
2. `docs/design/fleetdeck-v2/diff.md` rows D15 + §"Binding map" + §"Behaviour contract".
...
1. Summary bar (counts + spent / no credits spent) + privacy note verbatim; ordering most-constrained first.
2. Cards: header fields, `right` = "<ago> · <host|push>", banners/stale note verbatim, bars (labels, order, thresholds, resets), credits line, `seen on` line.
3. Sparkline from `row.history[].sd` (Claude rows only); collapsed/expanded default improvised + documented; collector errors panel.
4. Refresh → `credits({refresh:true})`; no polling (as today).
...
- [x] `verify/l8/report.json` allPass 36/36 (fixture mode); `verify/l8/live.json` all pass (66/66); live screenshots filed.
- [x] `improvised.md` entries + screenshots for every improvisation (I-L8-01 … I-L8-09).
- [x] Hooks provided are defined; hooks used are guarded. (Provided: none — the namespace is defined anyway. Used: none.)
- [x] `npm test` **green — 489/489, exit 0** on `origin/weave/fd-v2`; branch pushed; `REPORT.md` signed **Kunhild**; registry row `done` attempted — the endpoint answers HTTP 401 from this box (XYZ-2137).
=======
...
1. Pixel gate in fixture mode: `npm run design:diff -- --app <static server>/v2/index.html?fixture=1 --slice l8` → 36/36 ≤ 0.5 % → `docs/design/fleetdeck-v2/verify/l8/report.json` + PNGs committed.
2. Live-mode proof with the API stubbed: a Playwright script under `verify/l8/` that routes `/api/*` to the JSON fixtures in `docs/design/fleetdeck-v2/fixtures/api/` (plus hand-written variants for error/empty/offline states), drives every automatable `BEHAVIOUR.md` item, writes `verify/l8/live.json` (id, action, expectation, pass) and `verify/l8/live-<theme>.png`. Zero console errors.
3. Unit tests for pure logic (`test/v2-accounts.test.js`, node --test) where applicable.
4. `improvised.md` entries + screenshots for every improvisation.
5. `REPORT.md` signed **Kunhild**: behaviour checklist (every `BEHAVIOUR.md` item → done / partial / n.a. with reason), hooks provided/used, open follow-ups.
...
- Push with a fresh train-broker token per push (env only, never stored): `tok=$(curl -sf http://100.125.231.25:3131/api/ghtoken | python3 -c 'import json,sys;print(json.load(sys.stdin)["token"])'); GH_TOKEN=$tok git push origin agent-v2-l8` — 503 = `operator:gate`, block on that step only, keep committing.
