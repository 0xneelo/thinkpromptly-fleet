...
`--pin` is required (64 lowercase hex; missing or malformed = exit 2). The pin binds the sheet, the question id and the
question text, so the seat's question copied onto a sheet an agent posted does not verify with the seat's pin. Exit 0
only when the requested sheet, question id and served question reproduce the pin and the signature is valid, unexpired
and current. Exit 1: unanswered, unsigned, dismissed, timeout, error, invalid, expired, superseded, unconfigured or
pin-mismatch — `pin-mismatch`: the sheet, question id and question the deck serves now do not reproduce your pin
(another sheet, another question, or rewritten in fleet.db after you posted it); `invalid`: the signature does not
...
- **Sign-in** = the layer-1 password (scrypt hash in `~/.fleetdeck/operator.json`), rate-limited to 5 wrong tries
  ...
  "fleetdeck deck sign-in", then press Cmd-K to clear the scrollback. It refuses to run inside tmux or screen
  (`TMUX` or `STY` set): a multiplexer keeps scrollback any same-user process can read.
- **Signing** = deploy-class cards only (`deployClass: true` or an option key `deploy-*`). The deck signs only a
  ...
  agent shell can never raise a 1Password prompt.
