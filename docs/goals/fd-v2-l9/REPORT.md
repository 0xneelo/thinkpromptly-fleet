...
`validate()` is also exported and covered directly by six unit cases, because no reachable
`/api/machines` shape can produce the malformed input its drop branches exist for — the live
harness drives it through the real load path with non-string fields instead.

Everything else passes, and `test/v2-machines.test.js` contributes 55 of them.
