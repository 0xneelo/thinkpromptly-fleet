...
- [ ] `npm run design:diff -- --baseline` produces 36 PNGs; committed.
- [ ] `npm run design:diff -- --app <mock> --slice S0-selftest` → `allPass: true`, every mismatch ≤ 0.05 %; `verify/S0-selftest/report.json` committed.
- [ ] A second baseline run reproduces the first within 0.05 % (run it twice, diff the two reports; note the max delta in your report).
- [ ] `reviewer` subagent pass on the diff, findings fixed.
- [ ] README section + script header explain install, flags, thresholds, and the determinism hacks.
