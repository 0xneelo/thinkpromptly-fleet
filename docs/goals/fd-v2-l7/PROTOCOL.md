...
## Linear unreachable is never a blocker (added 2026-09-07 after FD-v2-s0/s1 stalled)

Box Linear OAuth tokens expire (`oauth_token_invalid_grant`). Two GPT workers read "a blocker that is
not a Linear issue does not exist" literally and marked their goals blocked after three failed posts.
Rule: when Linear errors, write the would-be issue or comment into `docs/goals/<slug>/LINEAR-PENDING.md`
(title, labels, body, timestamp), commit it, register with `task: "PENDING"`, and keep working. Retry once
per milestone; file the pending entries when Linear is back. The launch prompt must carry this line.
