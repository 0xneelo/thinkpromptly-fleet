...
- [x] `verify/l3/report.json` allPass **36/36** (max 0.033 %, Windows and Session full screen 0.000000 %); `verify/l3/live.json` **38/38** allPass; `live-dark.png` and `live-light.png` filed.
...
- [x] `npm test` **467/467**, run with nothing else going (the port-binding HTTP tests flake under contention, never the same one twice — see `REPORT.md`) (numbers and evidence in `REPORT.md`); branch pushed; `REPORT.md` signed **Gottlieb**; the registry row could not be written — the box's registry POST answers `401 unauthorized` (XYZ-2137), recorded on DECK-42.
