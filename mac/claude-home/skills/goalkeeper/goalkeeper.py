...
Three subcommands:
  ...
  goals        the goals the operator and this seat manage together (goals.json)
...
The data repo is $GOALKEEPER_HOME (default ~/goalkeeper). Every other repo is
...
FETCH_TIMEOUT = 60
SEAT_EMAIL = "goalkeeper@local"  # this CLI's own commits
OPERATOR_EMAIL = "operator@local"  # the deck writing on the operator's behalf
TRUSTED_EMAILS = {SEAT_EMAIL, OPERATOR_EMAIL}  # any other author is foreign
STATE_FILE = "sweep-state.json"  # what the seat itself last did; gitignored in the data repo
GOALS_FILE = "goals.json"  # the goals the operator and this seat manage together
...
def home_dir():
    return os.path.abspath(
        os.environ.get("GOALKEEPER_HOME") or os.path.expanduser("~/goalkeeper")
    )
...
def commit_file(home, relpath, message, by="goalkeeper"):
    """Stage and commit one file in the data repo. Returns False if git refused.

    `by="operator"` signs the commit `operator@local`: the deck wrote it on the
    operator's behalf, and the tamper check must not read it as someone else's edit.
    """
    ...
    if not run(["git", "-C", home, "add", "--", relpath])[0]:
        ...
        return False
    name, email = ("operator", OPERATOR_EMAIL) if by == "operator" else ("goalkeeper", SEAT_EMAIL)
    if not run(["git", "-C", home, "-c", "user.name=" + name,
                "-c", "user.email=" + email, "commit", "-m", message])[0]:
...
def goals_lock(home):
    """Hold one writer at a time over a jail: the seat's CLI and the deck's spawned one share it.

    The lock file lives in the tmpdir, not in the jail — an untracked file there would read
    as tamper on the next sweep.
    """
    digest = hashlib.sha256(os.path.abspath(home).encode("utf-8")).hexdigest()[:16]
    with open(os.path.join(tempfile.gettempdir(), "goalkeeper-%s.lock" % digest), "a") as fh:
...
def check_project(home, project):
    """None when the project is an alias from projects.json (or `all`), else the error."""
    aliases = [p["alias"] for p in load_projects(home)]
    if project == "all" or project in aliases:
        return None
    return ("error: unknown --project %r. Valid: %s\n"
            % (project, ", ".join(aliases + ["all"]) if aliases else "all"))


def by_operator(message, by):
    """Commit subjects say who wrote the record; the author email says it again."""
    return message + " (by operator)" if by == "operator" else message


def cmd_thread_add(args, home):
    error = check_project(home, args.project)
    if error:
    ...
    committed = commit_file(home, "thread.md",
                            by_operator("thread: %s · %s · %s" % (entry_id, args.horizon, args.project),
                                        args.by), by=args.by)
    print(entry_id)  # printed either way: the entry is in thread.md and has to be traceable
    return 0 if committed else 3


# -------------------------------------------------------------------- goals

STATUSES = ("open", "done", "parked", "dropped")
...
def load_goals(home):
    """The goals file as {"updated", "goals"}. Missing or empty → an empty one.

    A malformed file returns None: it is the operator's list, so the caller exits
    rather than overwrite anything it could not read.
    """
    try:
        with open(goals_path(home), encoding="utf-8") as fh:
            body = fh.read().strip()
    except OSError:
...
def read_goals(home):
    """load_goals, with the error message the CLI prints before exiting 2."""
    data = load_goals(home)
    if data is None:
        sys.stderr.write("error: %s is not a readable goals file — nothing was written\n"
                         % goals_path(home))
    return data


def write_goals(home, data):
    """Replace goals.json in one step; a half-written file would lose the list."""
    os.makedirs(home, exist_ok=True)
    fd, tmp = tempfile.mkstemp(dir=home, prefix=".goals-", suffix=".json")
    with os.fdopen(fd, "w", encoding="utf-8") as fh:
        fh.write(json.dumps(data, indent=2, ensure_ascii=False) + "\n")
    os.replace(tmp, goals_path(home))


def next_goal_id(goals):
    """GK-<max + 1>. Ids are never reused, so a dropped goal keeps its number."""
    highest = 0
    for goal in goals:
        match = re.match(r"^GK-(\d+)$", str(goal.get("id", "")))
        if match:
            highest = max(highest, int(match.group(1)))
    return "GK-%d" % (highest + 1)
...
def find_goal(data, goal_id):
    for goal in data["goals"]:
        if goal.get("id") == goal_id:
            return goal
    sys.stderr.write("error: unknown goal %r\n" % goal_id)
    return None


def split_refs(value):
    """`--refs a,b` → ["a", "b"]; `--refs ""` → [] (the way the deck clears them)."""
    return [ref.strip() for ref in (value or "").split(",") if ref.strip()]


def check_thread(home, thread_id):
    """None when thread.md holds that entry, else the error."""
    if any(entry["id"] == thread_id for entry in parse_thread(home)):
        return None
    return "error: unknown --thread %r — no such entry in thread.md\n" % thread_id
...
def cmd_goals_add(args, home):
    error = check_project(home, args.project) or (
        check_thread(home, args.thread) if args.thread else None)
    if error:
        sys.stderr.write(error)
        return 2
    data = read_goals(home)
    if data is None:
        return 2

    now = now_z()
    goal = {"id": next_goal_id(data["goals"]), "text": verbatim(args.text),
            "project": args.project, "horizon": args.horizon, "status": "open",
            "thread": args.thread or "", "refs": split_refs(args.refs),
            "state": args.state or "", "by": args.by, "created": now, "updated": now,
            "notes": []}
    data["goals"].append(goal)
    data["updated"] = now
    write_goals(home, data)
    committed = commit_file(home, GOALS_FILE,
                            by_operator("goals: %s add · %s · %s" % (goal["id"], args.project, args.horizon),
                                        args.by), by=args.by)
    print(goal["id"])  # the deck reads the last stdout line, committed or not
    return 0 if committed else 3
...
def cmd_goals_set(args, home):
    fields = [("status", args.status), ("state", args.state),
              ("refs", args.refs), ("thread", args.thread)]
    given = [(name, value) for name, value in fields if value is not None]
    if not given:
        sys.stderr.write("error: set needs at least one of --status, --state, --refs, --thread\n")
        return 2
    if args.thread:
        error = check_thread(home, args.thread)
        if error:
            sys.stderr.write(error)
            return 2
    data = read_goals(home)
    if data is None:
        return 2
    goal = find_goal(data, args.id)
    if goal is None:
        return 2

    before = json.dumps(goal, sort_keys=True)
    for name, value in given:
        goal[name] = split_refs(value) if name == "refs" else value
    if json.dumps(goal, sort_keys=True) == before:
        return 0  # an identical set changes nothing: no write, no empty commit
    now = now_z()
    goal["updated"] = now
    data["updated"] = now
    write_goals(home, data)
    return 0 if commit_file(home, GOALS_FILE,
                            by_operator("goals: %s set %s" % (args.id, ", ".join(
                                "%s=%s" % (name, short(value)) for name, value in given)), args.by),
                            by=args.by) else 3


def cmd_goals_note(args, home):
    data = read_goals(home)
    if data is None:
        return 2
    goal = find_goal(data, args.id)
    ...
    now = now_z()
    goal.setdefault("notes", []).append({"at": now, "by": args.by, "text": verbatim(args.text)})
    goal["updated"] = now
    data["updated"] = now
    write_goals(home, data)
    return 0 if commit_file(home, GOALS_FILE,
                            by_operator("goals: %s note" % args.id, args.by), by=args.by) else 3


def cmd_goals_list(args, home):
    data = read_goals(home)
    if data is None:
        return 2
    if args.json:
        ...
        return 0
    for goal in data["goals"]:
        print("%s  %s  %s  %s  %s" % (goal.get("id", ""), goal.get("status", ""),
                                      goal.get("horizon", ""), goal.get("project", ""),
                                      str(goal.get("text", ""))[:80]))
    return 0
...
def check_foreign_commits(home, baseline):
    """Commits since the baseline whose author is neither this seat nor the operator.

    The deck's writes are signed operator@local and are trusted; every other author
    is foreign.
    ...
    for line in out.splitlines():
        parts = line.split(US)
        if len(parts) != 4 or parts[1] in TRUSTED_EMAILS:
            continue
...
def detect_tamper(home):
    ...
    Three checks, all run before the sweep writes anything:
      ...
      (b) `foreign_commit`     — a commit since sweep-state.json's last_clean_sha whose
                                 author email is neither goalkeeper@local nor operator@local
...
def add_by(parser):
    parser.add_argument("--by", choices=["goalkeeper", "operator"], default="goalkeeper",
                        help="who is writing; `operator` is the deck, on the operator's behalf")
...
def main(argv=None):
    ...
    goals = subs.add_parser("goals", help="the goals the operator and this seat manage together")
    goals_subs = goals.add_subparsers(dest="goals_cmd")

    g_add = goals_subs.add_parser("add", help="add a goal to goals.json and commit")
    g_add.add_argument("text", help="the goal, verbatim")
    g_add.add_argument("--project", required=True, help="alias from projects.json, or `all`")
    g_add.add_argument("--horizon", choices=["day", "week"], required=True)
    g_add.add_argument("--thread", help="id of the thread.md entry this goal serves")
    g_add.add_argument("--refs", help="comma-separated refs, e.g. `G19,L4`")
    g_add.add_argument("--state", help="the goalkeeper's last finding")
    add_by(g_add)

    g_set = goals_subs.add_parser("set", help="change fields of one goal and commit")
    g_set.add_argument("id", help="goal id, e.g. GK-1")
    g_set.add_argument("--status", choices=list(STATUSES))
    g_set.add_argument("--state", help="the goalkeeper's last finding (\"\" clears it)")
    g_set.add_argument("--refs", help="comma-separated refs (\"\" clears them)")
    g_set.add_argument("--thread", help="id of the thread.md entry this goal serves")
    add_by(g_set)

    g_note = goals_subs.add_parser("note", help="append a note to one goal and commit")
    g_note.add_argument("id", help="goal id, e.g. GK-1")
    g_note.add_argument("text", help="the note, verbatim")
    add_by(g_note)

    g_list = goals_subs.add_parser("list", help="print the goals; never commits")
    g_list.add_argument("--json", action="store_true", help="print goals.json itself")
    ...
    if args.cmd == "thread":
        if args.thread_cmd != "add":
            thread.print_help()
            return 2
        with goals_lock(home):
            return cmd_thread_add(args, home)
    if args.cmd == "goals":
        handler = {"add": cmd_goals_add, "set": cmd_goals_set,
                   "note": cmd_goals_note, "list": cmd_goals_list}.get(args.goals_cmd)
        if handler is None:
            goals.print_help()
            return 2
        if handler is cmd_goals_list:
            return handler(args, home)
        with goals_lock(home):
            return handler(args, home)
    if args.cmd == "sweep":
