...
1. **Be in the right place.** cwd must be `~/goalkeeper`. If it is not, stop and tell
...
4. **Check the repo.** `git status` in `~/goalkeeper`. Uncommitted work from a previous
...
## Goals — managed together with the operator

The goals live in `goals.json`, next to `thread.md`. A goal is one thing to get done under a
direction. Four commands write it; never hand-edit the file.

```
goalkeeper.py goals add --project lowcapsxyz --horizon week --thread T-2026-09-10-2 \
  --refs G19 -- "ship the consensus page"      # prints the new id, e.g. GK-1
goalkeeper.py goals set GK-1 --status done --state "shipped, tests green"
goalkeeper.py goals note GK-1 -- "blocked on the deck restart"
goalkeeper.py goals list                        # or --json
```

- `--status` is `open`, `done`, `parked` or `dropped`. `--horizon` is `day` or `week`.
- `--thread` is the id of a `thread.md` entry, so a goal points at the operator's own words.
- `--state` is **your** last finding on that goal. Keep it short and current; the operator reads
  it on the deck.
- `--by operator` is reserved for the deck. The operator's goals page is
  `localhost:3131/app#goals`, which calls `POST /api/goals`, which runs these same commands.
  Those commits are signed `operator@local` and are **not** tamper.
- Every write commits `goals.json` at once. `goals list` never commits.

Run `goals list` at boot, next to reading `thread.md`: the thread is what the operator said, the
goals are what is open under it.
...
names `~/goalkeeper/` unless it is a plain read — `cat`, `head`, `tail`, `sed -n`, `grep`,
...
- **Never touch another project's working tree or source.** Every other repo is read-only
  ...
  see is needed. The only file you author lives in `~/goalkeeper/`.
