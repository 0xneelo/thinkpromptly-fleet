...
class TestInjectedTurns(unittest.TestCase):
    """`--strict-turns` (LINEAR-PENDING G-9). PLAN §3.3 stays the default."""
    ...
    def test_strict_flag_drops_injected_rows_only(self):
        """§3.3 is verbatim by default; strict removes the harness rows and nothing else."""
        rows = [
            {"type": "user", "timestamp": SINCE,
             "message": {"content": "redesign the consensus page"}},
            {"type": "user", "timestamp": SINCE,
             "message": {"content": "<task-notification>\n<task-id>x</task-id>"}},
            {"type": "user", "timestamp": SINCE,
             "message": {"content": "This session is being continued from a previous conversation"}},
        ]
        # every row passes the verbatim PLAN §3.3 filter ...
        self.assertEqual([gk.is_operator_turn(r) for r in rows], [True, True, True])
        # ... and strict keeps only the one the operator actually typed.
        kept = [r for r in rows
                if gk.is_operator_turn(r) and not gk.is_injected(r["message"]["content"])]
        self.assertEqual(len(kept), 1)
        self.assertEqual(kept[0]["message"]["content"], "redesign the consensus page")
