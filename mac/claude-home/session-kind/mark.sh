...
case "$1" in
  ...
  *)
    if ! valid_badge "$1"; then
      ...
      echo "mark.sh: expected '🎛 ORCHESTRATOR <N>', '🔬 RESEARCHER <N>', '🎨 DESIGN <N>' or '🔨 WORKER · <Name>' (no paths)." >&2
      echo "mark.sh: prefer 'mark.sh --orchestrator/--researcher/--design \"<topic>\"' / '--worker <Name>'." >&2
      exit 2
    fi
