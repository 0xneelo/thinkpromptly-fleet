...
template script body bytes  ${B(LOGIC)}
template script body sha256 ${sha(LOGIC)}
logic.js sha256             ${sha(LOGIC_OUT)}

round trip                  template.slice(0, bodyFrom) + template script body + template.slice(bodyTo)
result                      IDENTICAL to the source (${B(roundTrip)} bytes, sha256 ${sha(roundTrip)})
diff                        (empty — 0 differing bytes)

Note: the round trip is proven against the TEMPLATE SCRIPT BODY above. public/v2/logic.js is that body
after the T3 substitutions, so its sha256 differs by design; every moved literal is byte-identical and
quoted OUT -> IN in t3-substitutions.txt.

T3 substitution             ${subs.length} seed literal(s) moved to public/v2/fixture.js after the
                            round-trip check above.
logic.js emitted            ${B(LOGIC_OUT)} bytes, sha256 ${sha(LOGIC_OUT)}
`;

// Build outputs: what the app actually loads. These decide `--check`.
const files = [
...
// Evidence documents. Written by a compile, but deliberately NOT part of the
// staleness verdict. Two reasons. A `--check` must not write files, so it cannot
// heal them; and scripts/design-diff.mjs republishes verify/<slice>/ by renaming a
// staging directory over it, deleting these two on every pixel-gate run. Counting
// them as build outputs made `npm run v2:check` exit 1 purely because the pixel gate
// had run — a red check that says nothing about whether the app is stale.
const proofs = [
...
if (!CHECK) {
  for (const [rel, body] of proofs) {
    const abs = R(rel);
    const same = existsSync(abs) && readFileSync(abs, "utf8") === body;
    mkdirSync(path.dirname(abs), { recursive: true });
    if (!same) writeFileSync(abs, body);
    console.log(`${same ? "unchanged" : "wrote    "} ${rel} (${B(body)} bytes, evidence — not staleness-checked)`);
  }
}
...
for (const [rel, body] of files) {
  ...
  mkdirSync(path.dirname(abs), { recursive: true });
  if (!same) writeFileSync(abs, body);
  console.log(`${same ? "unchanged" : "wrote    "} ${rel} (${B(body)} bytes)`);
}
if (CHECK) {
